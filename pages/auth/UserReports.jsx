...
const UserSocialPost = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await axios.get(
        `https://dev.shiriki.org/api/get-reports/`,
        config
      );
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      const res = await axios.delete(`https://dev.shiriki.org/api/delete-post/`, {
   instance: id }, // Correct placement of the payload
        config
      );
        ...
        forums.map((forum, index) => (
          <View key={index} className="relative">
            ...
            <View className="absolute top-20 flex flex-row justify-between">
              <Ionicons2 size={26} className="ml-3" name="pencil" color="white" />
            </View>
          </View>
        ))
      )}
