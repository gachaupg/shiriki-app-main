import { Image, ScrollView, StyleSheet, Text, View } from "react-native";
import React from "react";
import Icon from "react-native-vector-icons/AntDesign";
import Ionicons from "react-native-vector-icons/FontAwesome";
import Ionicons1 from "react-native-vector-icons/Ionicons";
import Ionicons2 from "react-native-vector-icons/Feather";
import Ionicons3 from "react-native-vector-icons/MaterialCommunityIcons";
import Ionicons4 from "react-native-vector-icons/Entypo";
const SingleAlert = () => {
  const blogs = [
    {
      id: 1,
      title: "Social media influence",
      desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Vel, recusandae. Lorem ipsum dolor sit amet consectetur adipisicing elit. Vel, recusandae. Lorem ipsum dolor sit amet consectetur adipisicing elit. Vel, recusandae.",
      image:
        "https://www.searchenginejournal.com/wp-content/uploads/2021/09/16-reasons-why-social-media-is-important-to-your-company-616d3200e6dc6-sej.png",
    },
    {
      id: 1,
      title: "Politics Debate",
      desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Vel, recusandae.",
      image:
        "data:image/jpeg;base64,",
    },
    {
      id: 1,
      title: "Dollar to shilling drop",
      desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Vel, recusandae.",
      image:
        "https://www.theeastafrican.co.ke/resource/blob/4367498/bb9f3be1fdca3318344ebdb4d5f936ec/shillingke-data.jpg",
    },
  ];
  return (
    <ScrollView>
      <Image
        style={styles.image}
        source={{
          uri: "https://www.searchenginejournal.com/wp-content/uploads/2021/09/16-reasons-why-social-media-is-important-to-your-company-616d3200e6dc6-sej.png",
        }}
      />
      <View style={{ padding: 10 }}>
        <Text style={{ marginLeft: 10, color: "green", fontSize: 18 }}>
          Social media influence
        </Text>
        <Text style={{display:'flex',alignItems:'center',justifyContent:'center'}}>
          Lorem, ipsum dolor sit amet consectetur adipisicing elit. Maiores,kkkk
          tenetur praesentium in quisquam quae quam repellat fuga dolores omnjjjis
          quia distinctio, fugit laboriosam. Deleniti reiciendis, ut aliquid hhhjj
          accusamus similique iste vel delectus et rerum, beatae doloremquejjjjjj
          deserunt ab, porro quis! Consequuntur dolore, quas, eveniet doloribus,
          nobis ipsam ea nisi enim non quaerat ipsum. Libero animi explicabo
          quod id blanditiis praesentium molestias debitis, sint corporis
          placeat maiores nam excepturi unde in. Similique aliquid, voluptates
          omnis deleniti recusandae maxime tenetur explicabo. Nisi vel veniam
          qui deleniti assumenda. Totam nisi cum ipsam eaque quaerat minima
          mollitia minus nobis quis! Vero optio quisquam placeat deserunt
          dolores eius dolorem dicta impedit illo molestiae ut accusamus vel,
          adipisci atque eaque, officia, veritatis aperiam fugiat quibusdam
          omnis modi! Esse, at. Deserunt facere quo, quos ratione iusto tempora
        </Text>
         <View
          style={{
            display: "flex",
            flexDirection: "row",
            alignItems: "center",
            justifyContent: "space-between",
            gap: 40,
            marginTop: 16,
          }}
        >
          <View>
            <Icon name="like2" size={24} color="grey" />
            <Text style={{ color: "grey", fontSize: 12 }}>200</Text>
          </View>
          <View>
            <Icon name="dislike2" size={24} color="grey" />
            <Text style={{ color: "grey", fontSize: 12 }}>200</Text>
          </View>
          <Ionicons name="share" size={24} color="grey" />
          <Ionicons1 name="add-circle" size={24} color="grey" />
        </View>
      </View>
    </ScrollView>
  );
};

export default SingleAlert;

const styles = StyleSheet.create({
  image: {
    height: 300,
    width: "100%",
  },
});
