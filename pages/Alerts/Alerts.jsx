    ...
    <ScrollView style={{ padding: 25 }}>
      ...
      <View style={{marginTop:20}}> 
        {blogs.map((blog, index) => (
          <TouchableOpacity
          onPress={() =>
            navigation.navigate("SingleAlert", { id: blog.id })
          } 
          key={index} style={styles.blogContainer}>
        ...
        >
          ...
          <Ionicons1 name="add-circle" size={15} color="grey" />
        </View>
            <View style={{display:'flex',flexDirection:'row',alignItems:'center',justifyContent:'space-between',marginTop:10}}>
              <Button title="Read more" color='green'/>
              <Text>Posted on 12/06/2024</Text>
            </View>
          </TouchableOpacity>
        ))}
      </View>
